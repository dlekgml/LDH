...
function rolling(obj); 주사위 굴리기 위한 함수 - 게임진행 
   ...
   function meeple_move(); 플레이어 말을 주사위값에 따라 이동시키는 함수 - 게임진행
   function 다음턴(who); 다음차례가 누구인지 정해주는 함수 - 게임진행
        매개변수 - who : 현재턴 플레이어 번호

   function 말위치변경(gamer, old_location); 말이 이동할경우 이전위치와 현재위치 - 게임진행
        매개변수 - gamer : 현재 플레이어객체
                  old_location : 현재플레이어의 주사위 값에 따라 이동하기전의 위치(num)  
...
// 함수정의
function rolling(obj){ // 주사위 생선된후 버튼을 클릭하면 주사위가
...
function 다음턴(who){
    ...
    if(island_.includes(who+1)){
        ...
        if(player_list[who].drift_turn == 0){ // 무인도 남은턴이 0 이면 island 배열에서 제거
            island_.splice(island.indexOf(who+1),1);
        }
        return 다음턴(who+1);
    }    
...
function game_todo(location , gamer){
 // location 매개변수는 zone 클래스들중 몇번째 zone클래스인지 인덱스값 있음
 // location의 값은 몇번째 zone 클래스인지 알수도 있지만, zone 배열의
 // 구역객체의 인덱스로도 사용가능
    ...
    }else if(city.owner == ''){ // 도시의 주인이 없는 경우
        if(confirm(`${city.name}의 매입가는 
            ${city.purchase}만원입니다. \n 매입하시겠습니까?`)){
                if(gamer.money < city.purchase){ // 토지매입가 보다 플레이어 자금이 적으면
                   alert("자금이 부족합니다.");
                   return; 
                }
    ...
    function airport_move(){
        ...
        if(탑승객 !=0 ){ // 탑승객 변수에 0이 있다면 현재 이용할수 있는 플레이어가 없다.
             ...
             // meeple-move에서 turn이 변경되기때문에 공항에 도착한 
            ...
            if(클릭한도시번호 == 24){  // 24는 공항 num이다.
                 return;  // 공항을 클릭하면 아무 작업도 하지 않는다.
            }
            ...
            game_todo(zone_location , gamer); // 클릭한 위치에서 할일 실행(매입,기금,무인도,출발지 등);
            탑승객=0;
        }
    }