/*
    function zone_draw(); 부루마블 게임판에 도시명, 이미지, 색상을 출력 - 게임시작전
...
function find_location( n ){ // 플레이어 말이 표시될 위치 또는 이동할 위치 찾기
    ...
    $(".zone").each(function(idx, item){
        ...
        // each 안에 만든 익명함수만 종료시키기 때문에 find_lacation 함수를
        // 통해 return(반환) 하는게 불가능하다. 그래서 값을 index변수에
        // 저장하여 index변수를 반환 시켜준 것이다.
        }
    });
    return index;
}
...
function change_pcl(){ // 플레이어가 자신의 말 색상을 변경할 경우 실행
    ...
    var num=$(this).attr("id").substring(3); // attr 은 태그의 속성 - id,class,name 등등
    var gamer = player_list[num-1] // 색상 변경한 플레이어 객체

    gamer.color=new_color;
    // 자신의 말 찾기, num 변수에는 플레이어의 번호가 저장되있다.
    for(var i=0; i<$(".meeple").length; i++){
        if($(".meeple").eq(i).data("pn")==num){
           $(".meeple").eq(i).css("color",new_color);
           break; // 플레이어 말 찾아서 색변경 했으니까 반복문 종료
        }
    }
}

$(function(){
    // 현재상황 - json 파일내용은 어제다루었던 zone배열안의 내용과 같다.
    // getJSON으로 json 파일을 읽어와서 zone 배열에 저장
    // zone 배열의 내용은 어제와 동일하다.
    // zone_draw()함수는 zone 배열의 값을 가지고 와서 화면에 표시한다.
    // 하지만 zone_draw() 함수가 정상적으로 동작하지 못하고 있다.
    // getJSON의 function(data) { } 안으로 zone_draw() 함수를 넣어주면
    // 정상적으로 실행된다.
    // 비동기문제 발생
    // 비동기처리 코드가 실행이 완료되면 그다음 실행 될수 있게 하는 방법
    // async, await
    // async를 비동기 함수 앞에 붙여준다.
