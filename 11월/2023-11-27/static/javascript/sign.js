$(function(){
    ...
    $(".ability_list").on("click",function(){
        ...
        if(chk_count==all_check) // 체크된 갯수와 총 갯수를 비교
            $("#all").prop("checked",true); // 체크갯수와 총 갯수가 같담면 전체 체크박스에 체크
        else
    ...
    $("#myphone").on("keyup",function(){
       ...
       // num.replace(/[^0-9]/g, '').replace(/^(\d{3})(\d{4})(\d{4})$/, `$1-$2-$3`);
    ...
    $("#join").on("click",function(){
        ...
        // var chk = $(".ability_list").eq(0).attr("checked");
        // alert(chk);
        var len = $(".avility_list:cheked").length;
        alert(len);

        let chk =''; // 체크박스에 체크된 값들 저장 변수
        $.each($(".ability_list:cheked"), function(){ // 체크되어있는 checkbox만 반복문처리
            chk += $(this).val() + " ";
