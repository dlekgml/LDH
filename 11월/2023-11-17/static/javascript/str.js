...
// }
// // 4. indexOf는 문자열의 첫번째부터 검색을 하기 때문에
...
// word = "i like css like like";
// word2 = word.replace(/like/gi, "love"); // gi : like 전체 love로 바꿔줌
// console.log(word2);
...
// word ="banana \napple \nmember!!";   // \ 단어랑 붙여 써야함
// console.log(word.match(/^apple/gm)); // ^apple : 문장에서 apple로 시작하는지 검사
...
//     이메일 유효 검사 정규표현식
//     ^[a-zA-Z0-9+-\_.]+@[a-zA-Z0-9]Z+\.[a-zA-Z]+$

//     영어숫자 - abc_11@naver.com

//     /^010-?([0-9]{4})-?([0-9]{4})$/  -> {4} : 4자리를 의미 
...
// // 전화번호에서 1234-1234 부분을 xxxx-1234 로 변경
...
// // 7. 문자열추출 - substring()
// console.log( word.substring(0,7)); // 0 부터 7 이전까지 추출
// console.log( word.substring(4)); // 4 부터 끝까지 추출
...
// console.log( word.slice(-3,5)); // 음수,양수 같이 사용안됨
...
// // 강동욱이 보유한 현금에 서종우의 돈을 뻬면 얼마인가?? - split, slice 또는 substring, replace
...
//     .lastIndexOf(문자열) - 문자열의 끝에서 문자 또는 문자열 몇번째 있는지 검색
//     .includes(문자열) - 문자열에 특정 문자 또는 문자열의 존재여부(true, false)
//     .replace(원본문자열, 변환문자열) - 문자열에서 특정문자 또는 문자열을 지정한 문자 또는 문자열로
...
let data = info.split(" - "); // 띄워야 공백이 없음
// 출력 - 이름  직업
...
console.log(parseInt(age)); // parseInt() - 앞쪽에 있는 숫자만 정수로 변환
